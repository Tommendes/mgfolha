...
use common\models\UserDomainsSearch;
...
/**
 * UserDomainsController implements the CRUD actions for UserDomains model.
 */
class UserDomainsController extends Controller {

    public $gestor = 0;
    public $userdominios = 0;

    const CLASS_VERB_NAME = 'Cliente';
    const CLASS_VERB_NAME_PL = 'Clientes';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->userdominios = Yii::$app->user->identity->administrador >= 1;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->userdominios >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->userdominios >= 2);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->userdominios >= 3);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->userdominios >= 4);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all UserDomains models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new UserDomainsSearch();
    ...
    /**
     * Displays a single UserDomains model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing UserDomains model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing UserDomains model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = UserDomains::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing UserDomains model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = UserDomains::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = UserDomains::STATUS_ATIVO;
    ...
    /**
     * Finds the UserDomains model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return UserDomains the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = UserDomains::find()
                            ->where([
