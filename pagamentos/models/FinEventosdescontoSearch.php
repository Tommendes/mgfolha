...
/**
 * FinEventosdescontoSearch represents the model behind the search form of `pagamentos\models\FinEventosdesconto`.
 */
class FinEventosdescontoSearch extends FinEventosdesconto {
    ...
    public function rules() {
        return [
            [['id', 'status', 'evento', 'created_at', 'updated_at', 'id_fin_eventos', 'id_fin_eventosdesconto'], 'integer'],
            [['slug', 'dominio', 'id_fin_eventos', 'id_fin_eventosdesconto'], 'safe'],
        ];
    }
    ...
    public function search($params, $id_fin_eventos = null) {
        $query = FinEventosdesconto::find();
        $evts_tbl = 'a';
        $evts_base = 'b';

        // add conditions that should always apply here
        $query->join('join', FinEventos::tableName() . ' ' . $evts_tbl, $evts_tbl . '.id = ' . self::tableName() . '.id_fin_eventos');
        $query->join('join', FinEventos::tableName() . ' ' . $evts_base, $evts_base . '.id = ' . self::tableName() . '.id_fin_eventosdesconto');
...
//            $this::tableName() . '.id_fin_eventosdesconto' => $this->id_fin_eventosdesconto,
        ]);
        ...
        $query->andFilterWhere(['or',
            ['like', $evts_base . '.evento_nome', $this->id_fin_eventosdesconto],
            ['like', $evts_base . '.id_evento', $this->id_fin_eventosdesconto],
        ]);
