...
use Yii;
use pagamentos\models\CadLocaltrabalho;
use pagamentos\models\CadLocaltrabalhoSearch;
...
/**
 * CadLocaltrabalhoController implements the CRUD actions for CadLocaltrabalho model.
 */
class CadLocaltrabalhoController extends Controller {

    public $gestor = 0;
    public $cadlocaltrabalho = 0;

    const CLASS_VERB_NAME = 'Local de trabalho';
    const CLASS_VERB_NAME_PL = 'Locais de trabalho';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadlocaltrabalho = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadlocaltrabalho >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadlocaltrabalho >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadlocaltrabalho >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadlocaltrabalho >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadLocaltrabalho models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadLocaltrabalhoSearch();
    ...
    /**
     * Displays a single CadLocaltrabalho model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new CadLocaltrabalho model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp = null) {
        $model = new CadLocaltrabalho();
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadLocaltrabalho model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadLocaltrabalho model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadLocaltrabalho::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadLocaltrabalho model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadLocaltrabalho::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadLocaltrabalho::STATUS_ATIVO;
    ...
    /**
     * Finds the CadLocaltrabalho model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadLocaltrabalho the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadLocaltrabalho::find()
                            ->where([
            ...
            if (($model = CadLocaltrabalho::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
