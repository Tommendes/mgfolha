...
use pagamentos\models\FinBasefixaSearch;
...
/**
 * FinBasefixaController implements the CRUD actions for FinBasefixa model.
 */
class FinBasefixaController extends Controller {

    public $gestor = 0;
    public $finbasefixa = 0;

    const CLASS_VERB_NAME = 'Base fixa';
    const CLASS_VERB_NAME_PL = 'Bases fixas';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->finbasefixa = Yii::$app->user->identity->financeiro;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->finbasefixa >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixa >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixa >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixa >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinBasefixa models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new FinBasefixaSearch();
    ...
    /**
     * Displays a single FinBasefixa model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing FinBasefixa model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing FinBasefixa model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = FinBasefixa::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing FinBasefixa model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = FinBasefixa::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinBasefixa::STATUS_ATIVO;
    ...
    /**
     * Finds the FinBasefixa model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return FinBasefixa the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = FinBasefixa::find()
                            ->where([
