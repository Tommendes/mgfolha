...
use pagamentos\models\FinEventosbaseSearch;
...
/**
 * FinEventosbaseController implements the CRUD actions for FinEventosbase model.
 */
class FinEventosbaseController extends Controller {

    public $gestor = 0;
    public $fineventosbase = 0;

    const CLASS_VERB_NAME = 'Evento da base';
    const CLASS_VERB_NAME_PL = 'Eventos da base';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->fineventosbase = Yii::$app->user->identity->financeiro;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->fineventosbase >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventosbase >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventosbase >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventosbase >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinEventosbase models.
     * @return mixed
     */
    public function actionIndex(array $cl = null) {
        $searchModel = new FinEventosbaseSearch();
    ...
    /**
     * Displays a single FinEventosbase model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new FinEventosbase model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp) {
        $model = new FinEventosbase();
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing FinEventosbase model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing FinEventosbase model.
    ...
    public function actionDlt($id) {
        ...
        $model->status = FinEventosbase::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing FinEventosbase model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = FinEventosbase::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinEventosbase::STATUS_ATIVO;
    ...
    /**
     * Finds the FinEventosbase model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return FinEventosbase the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = FinEventosbase::find()
                            ->where([
            ...
            if (($model = FinEventosbase::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
