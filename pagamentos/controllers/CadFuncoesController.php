...
use pagamentos\models\CadFuncoesSearch;
...
/**
 * CadFuncoesController implements the CRUD actions for CadFuncoes model.
 */
class CadFuncoesController extends Controller {
    ...
    const CLASS_VERB_NAME = 'Função';
    const CLASS_VERB_NAME_PL = 'Funções';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadfuncoes = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadfuncoes >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadfuncoes >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadfuncoes >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadfuncoes >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadFuncoes models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadFuncoesSearch();
    ...
    /**
     * Displays a single CadFuncoes model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadFuncoes model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadFuncoes model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadFuncoes::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadFuncoes model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadFuncoes::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadFuncoes::STATUS_ATIVO;
    ...
    /**
     * Finds the CadFuncoes model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadFuncoes the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadFuncoes::find()
                            ->where([
