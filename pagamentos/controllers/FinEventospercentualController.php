...
use pagamentos\models\FinEventospercentualSearch;
...
/**
 * FinEventospercentualController implements the CRUD actions for FinEventospercentual model.
 */
class FinEventospercentualController extends Controller {

    public $gestor = 0;
    public $fineventospercentual = 0;

    const CLASS_VERB_NAME = 'Percentual da base';
    const CLASS_VERB_NAME_PL = 'Percentuais da base';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->fineventospercentual = Yii::$app->user->identity->financeiro;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->fineventospercentual >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventospercentual >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventospercentual >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->fineventospercentual >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinEventospercentual models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new FinEventospercentualSearch();
    ...
    /**
     * Displays a single FinEventospercentual model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
            ...
            return $this->redirect(['/fin-eventos/view', 'id' => $model->id_fin_eventos]);
    ...
    /**
     * Creates a new FinEventospercentual model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp) {
        $model = new FinEventospercentual();
        $model->id_fin_eventos = $cp;
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing FinEventospercentual model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing FinEventospercentual model.
    ...
    public function actionDlt($id) {
        ...
        $model->status = FinEventospercentual::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing FinEventospercentual model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = FinEventospercentual::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinEventospercentual::STATUS_ATIVO;
    ...
    /**
     * Finds the FinEventospercentual model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return FinEventospercentual the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = FinEventospercentual::find()
                            ->where([
            ...
            if (($model = FinEventospercentual::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
