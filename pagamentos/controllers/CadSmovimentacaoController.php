...
use pagamentos\models\CadSmovimentacaoSearch;
use pagamentos\models\CadSmovimentacao;

/**
 * CadSmovimentacaoController implements the CRUD actions for CadSmovimentacao model.
 */
class CadSmovimentacaoController extends Controller {

    public $gestor = 0;
    public $cadsmovimentacao = 0;

    const CLASS_VERB_NAME = 'Movimentação do servidor';
    const CLASS_VERB_NAME_PL = 'Movimentações do servidor';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadsmovimentacao = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadsmovimentacao >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl', 'z'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadsmovimentacao >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadsmovimentacao >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadsmovimentacao >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    public function actionIndex($id_cad_servidores = null, $modal = false) {
        if (!is_null($id_cad_servidores)):
            ...
            Yii::$app->session->setFlash('warning', Yii::t('yii', 'Para acessar um registro de movimentação, localize primeiro o cadastro do servidor responsável'));
            return $this->redirect([Url::to('/cad-servidores')]);
        endif;
    }

    /**
     * Displays a single CadSmovimentacao model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new CadSmovimentacao model.
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadSmovimentacao model.
     * If update is successful, the browser will be redirected to the 'view' page.
     * @param integer $id
     * @return mixed
     * @throws NotFoundHttpException if the model cannot be found
     */
    public function actionU($id, $modal = false, $mv = null) {
...
//        }
    ...
    /**
     * Deletes an existing CadSmovimentacao model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadSmovimentacao::STATUS_CANCELADO;
...
//          registra evento no log do sistema
        ...
        if ($model->save(false)) {
            $data = ['mess' => Yii::t('yii', 'Successfully canceled record.'), 'class' => 'success', 'stat' => true, 'reloadPage' => true];
        } else {
    ...
    /**
     * Duplicates an existing CadSmovimentacao model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadSmovimentacao::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadSmovimentacao::STATUS_ATIVO;
    ...
    /**
     * Retorna a lista de códigos de retorno de movimentação.
     * @param integer $id
     * @return mixed
     */
    public function actionZ($id) {
        $posts = \common\models\Listas::getCodsRetorno($id);
    ...
    /**
     * Finds the CadSmovimentacao model based on its primary key value.
    ...
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadSmovimentacao::find()
                            ->where([
            ...
            if (($model = CadSmovimentacao::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
