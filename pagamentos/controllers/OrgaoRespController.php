...
/**
 * OrgaoRespController implements the CRUD actions for OrgaoResp model.
 */
class OrgaoRespController extends Controller {

    public $gestor = 0;
    public $administrador = 0;

    const CLASS_VERB_NAME = 'Responsável DIRF';
    const CLASS_VERB_NAME_PL = 'Responsáveis DIRF';
    ...
    /**
     * Displays a single OrgaoResp model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing OrgaoResp model.
...
//        }
    ...
    /**
     * Deletes an existing OrgaoResp model.
...
//        $this->findModel($id)->delete();
    ...
    /**
     * Duplicates an existing OrgaoResp model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = OrgaoResp::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = OrgaoResp::STATUS_ATIVO;
    ...
    /**
     * Finds the OrgaoResp model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return OrgaoResp the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = OrgaoResp::find()
                            ->where([
                                'id' => $id,
                            ])->asArray()->one()) !== null) {
                return $model;
