...
use pagamentos\models\CadDepartamentosSearch;
...
/**
 * CadDepartamentosController implements the CRUD actions for CadDepartamentos model.
 */
class CadDepartamentosController extends Controller {

    public $gestor = 0;
    public $caddepartamentos = 0;

    const CLASS_VERB_NAME = 'Departamento';
    const CLASS_VERB_NAME_PL = 'Departamentos';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->caddepartamentos = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->caddepartamentos >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->caddepartamentos >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->caddepartamentos >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->caddepartamentos >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadDepartamentos models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadDepartamentosSearch();
    ...
    /**
     * Displays a single CadDepartamentos model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new CadDepartamentos model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp = null) {
        $model = new CadDepartamentos();
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadDepartamentos model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadDepartamentos model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadDepartamentos::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadDepartamentos model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadDepartamentos::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadDepartamentos::STATUS_ATIVO;
    ...
    /**
     * Finds the CadDepartamentos model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadDepartamentos the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadDepartamentos::find()
                            ->where([
            ...
            if (($model = CadDepartamentos::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
