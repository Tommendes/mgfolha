...
class FinReferenciasController extends Controller
{
            ...
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        ...
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finreferencias >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finreferencias >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finreferencias >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinReferencias models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new FinReferenciasSearch();
    ...
    /**
     * Creates a new FinReferencias model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp = null)
    {
        $model = new FinReferencias();
    ...
    /**
     * Updates an existing FinReferencias model.
    ...
    /**
     * Deletes an existing FinReferencias model.
    ...
    public function actionDlt($id)
    {
        ...
        $model->status = FinReferencias::STATUS_CANCELADO;
    ...
    /**
     * Duplicates an existing FinReferencias model.
    ...
    {
        $model = $this->findModel($id);
        $id_n = FinReferencias::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinReferencias::STATUS_ATIVO;
    ...
    /**
     * Retorna os valores em BD.get_vcto_basico
     * @param type $id
     * @param type $v_ano
     * @param type $v_mes
     ...
     * @return type
     */
    public static function actionZ($id, $v_ano = null, $v_mes = null, $v_parcela = null)
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        return self::getVctoBasico($id, $v_ano, $v_mes, $v_parcela);
    }
     ...
     * @return type
     */
    public static function getVctoBasico($id, $v_ano = null, $v_mes = null, $v_parcela = null)
    {
        ...
        $db = isset(Yii::$app->user->identity->cliente) ? strtolower(str_replace('_', '', Yii::$app->user->identity->cliente)) : 'db';
        $const = "call mgfolha_$db.get_vcto_basico('$dominio', '$v_ano', '$v_mes', '$v_parcela', '$id')";
        // SisEventsController::registrarEvento("$const", 'getVctoBasico', Yii::$app->user->identity->username, "");
        // return $const;
        return Yii::$app->db->createCommand($const)->queryOne();
    }
