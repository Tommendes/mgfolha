...
use pagamentos\models\CadBconveniosSearch;
...
/**
 * CadBconveniosController implements the CRUD actions for CadBconvenios model.
 */
class CadBconveniosController extends Controller {

    public $gestor = 0;
    public $cadbconvenios = 0;

    const CLASS_VERB_NAME = 'Convênio bancário';
    const CLASS_VERB_NAME_PL = 'Convênios bancários';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadbconvenios = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadbconvenios >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadbconvenios >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadbconvenios >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadbconvenios >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadBconvenios models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadBconveniosSearch();
    ...
    /**
     * Displays a single CadBconvenios model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadBconvenios model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadBconvenios model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadBconvenios::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadBconvenios model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadBconvenios::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadBconvenios::STATUS_ATIVO;
    ...
    /**
     * Finds the CadBconvenios model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadBconvenios the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadBconvenios::find()
                            ->where([
            ...
            if (($model = CadBconvenios::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
