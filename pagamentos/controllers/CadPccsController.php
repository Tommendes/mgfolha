...
use pagamentos\models\CadPccsSearch;
...
/**
 * CadPccsController implements the CRUD actions for CadPccs model.
 */
class CadPccsController extends Controller {

    public $gestor = 0;
    public $cadpccs = 0;

    const CLASS_VERB_NAME = 'Plano de cargos e carreiras';
    const CLASS_VERB_NAME_PL = 'Planos de cargos e carreiras';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadpccs = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadpccs >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadpccs >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadpccs >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadpccs >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadPccs models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadPccsSearch();
    ...
    /**
     * Displays a single CadPccs model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadPccs model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadPccs model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadPccs::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadPccs model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadPccs::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadPccs::STATUS_ATIVO;
    ...
    /**
     * Finds the CadPccs model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadPccs the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadPccs::find()
                            ->where([
