...
use pagamentos\models\FinBasefixaeventosSearch;
...
/**
 * FinBasefixaeventosController implements the CRUD actions for FinBasefixaeventos model.
 */
class FinBasefixaeventosController extends Controller {

    public $gestor = 0;
    public $finbasefixaeventos = 0;

    const CLASS_VERB_NAME = 'FinBasefixaeventos';
    const CLASS_VERB_NAME_PL = 'FinBasefixaeventoss';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->finbasefixaeventos = Yii::$app->user->identity->finbasefixaeventos;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->finbasefixaeventos >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixaeventos >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixaeventos >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixaeventos >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinBasefixaeventos models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new FinBasefixaeventosSearch();
    ...
    /**
     * Displays a single FinBasefixaeventos model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new FinBasefixaeventos model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp = null) {
        $model = new FinBasefixaeventos();
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing FinBasefixaeventos model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing FinBasefixaeventos model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = FinBasefixaeventos::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing FinBasefixaeventos model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = FinBasefixaeventos::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinBasefixaeventos::STATUS_ATIVO;
    ...
    /**
     * Finds the FinBasefixaeventos model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return FinBasefixaeventos the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = FinBasefixaeventos::find()
                            ->where([
            ...
            if (($model = FinBasefixaeventos::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
