...
use pagamentos\models\CadCentrosSearch;
...
/**
 * CadCentrosController implements the CRUD actions for CadCentros model.
 */
class CadCentrosController extends Controller {

    public $gestor = 0;
    public $cadcentros = 0;

    const CLASS_VERB_NAME = 'Centro';
    const CLASS_VERB_NAME_PL = 'Centros';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadcentros = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadcentros >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcentros >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcentros >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcentros >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadCentros models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadCentrosSearch();
    ...
    /**
     * Displays a single CadCentros model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadCentros model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadCentros model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadCentros::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadCentros model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadCentros::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadCentros::STATUS_ATIVO;
    ...
    /**
     * Finds the CadCentros model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadCentros the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadCentros::find()
                            ->where([
