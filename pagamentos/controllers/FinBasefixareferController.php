...
use pagamentos\models\FinBasefixareferSearch;
...
/**
 * FinBasefixareferController implements the CRUD actions for FinBasefixarefer model.
 */
class FinBasefixareferController extends Controller {

    public $gestor = 0;
    public $finbasefixarefer = 0;

    const CLASS_VERB_NAME = 'Referencia da base fixa';
    const CLASS_VERB_NAME_PL = 'Referencias da base fixa';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->finbasefixarefer = Yii::$app->user->identity->financeiro;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->finbasefixarefer >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixarefer >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixarefer >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->finbasefixarefer >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all FinBasefixarefer models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new FinBasefixareferSearch();
    ...
    /**
     * Displays a single FinBasefixarefer model.
     * @param integer $id
     * @return mixed
...
//          registra evento no log do sistema
    ...
    /**
     * Creates a new FinBasefixarefer model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionC($modal = false, $cp = null) {
        $model = new FinBasefixarefer();
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing FinBasefixarefer model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing FinBasefixarefer model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = FinBasefixarefer::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing FinBasefixarefer model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = FinBasefixarefer::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = FinBasefixarefer::STATUS_ATIVO;
    ...
    /**
     * Finds the FinBasefixarefer model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return FinBasefixarefer the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = FinBasefixarefer::find()
                            ->where([
            ...
            if (($model = FinBasefixarefer::findOne([
                        is_numeric($id) ? 'id' : 'slug' => $id,
                        'dominio' => Yii::$app->user->identity->dominio,
                    ])) !== null) {
                return $model;
