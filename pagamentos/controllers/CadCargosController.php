...
use pagamentos\models\CadCargosSearch;
...
/**
 * CadCargosController implements the CRUD actions for CadCargos model.
 */
class CadCargosController extends Controller {

    public $gestor = 0;
    public $cadcargos = 0;

    const CLASS_VERB_NAME = 'Cargo';
    const CLASS_VERB_NAME_PL = 'Cargos';
    ...
    public function behaviors() {
        if (!Yii::$app->user->isGuest) {
            $this->cadcargos = Yii::$app->user->identity->cadastros;
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && $this->cadcargos >= 1;
                        }
                    ],
                    [
                        'actions' => ['c', 'dpl'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcargos >= 2 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['u'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcargos >= 3 || $this->gestor >= 1);
                        }
                    ],
                    [
                        'actions' => ['dlt'],
                        'allow' => true,
                        'matchCallback' => function () {
                            return !Yii::$app->user->isGuest && ($this->cadcargos >= 4 || $this->gestor >= 1);
                        }
                    ],
                ],
            ],
    ...
    /**
     * Lists all CadCargos models.
     * @return mixed
     */
    public function actionIndex() {
        $searchModel = new CadCargosSearch();
    ...
    /**
     * Displays a single CadCargos model.
     * @param integer $id
     * @return mixed
...
//          gera evento para ser gravado no log do sistema 
    ...
    /**
     * Updates an existing CadCargos model.
...
//          registra evento no log do sistema
    ...
    /**
     * Deletes an existing CadCargos model.
...
//        $this->findModel($id)->delete();
        ...
        $model->status = CadCargos::STATUS_CANCELADO;
...
//          registra evento no log do sistema
    ...
    /**
     * Duplicates an existing CadCargos model.
    ...
    public function actionDpl($id) {
        $model = $this->findModel($id);
        $id_n = CadCargos::find()->max('id') + 1;
        $model->id = $id_n;
        $model->status = CadCargos::STATUS_ATIVO;
    ...
    /**
     * Finds the CadCargos model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return CadCargos the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $array = false) {
        if ($array) {
            if (($model = CadCargos::find()
                            ->where([
