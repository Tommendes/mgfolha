...
$this->params['breadcrumbs'][] = Yii::t('yii', 'Todas as ') . strtolower(SisReviewsController::CLASS_VERB_NAME_PL);
?>
<div class="<?= SisReviewsController::CLASS_VERB_NAME ?>-index form-bg form-transparence">    
    <div class="d-flex align-items-center p-3 my-3 text-white-50 bg-red rounded shadow-sm">
        <h1>Novidades e revisões do sistema web</h1>
    </div>
    <div class="my-3 p-3 bg-white rounded shadow-sm">
        ...
        foreach ($model as $linhas) {
            if (count($linha = explode(";", $linhas)) == 5) {
                ...
                $data = date('d/m/Y H:i:s', $unix_time);
                ?>
                <div class="media text-muted pt-3">
                    ...
                    <p class="media-body pb-3 mb-0 border-bottom border-gray">
                        <strong class="d-block"><?= "Revisão #$hash em $data: $assunto" ?></strong>
                        <?= strlen(trim($descricao)) > 0 ? $descricao : '' ?>
                    </p>
                </div>
                <?php
            }
