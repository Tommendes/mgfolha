...
$this->title = SisReviewsController::CLASS_VERB_NAME_PL;
$this->params['breadcrumbs'][] = $this->title;

$modal = !isset($modal) ? false : $modal;
?>
<div class="sis-reviews-index">
        ...
        'buttons' => [
            'view' => function ($url, $model) {
                return Html::button('<span class="fa fa-eye"></span> ', [
                            'value' => Url::to(['sis-reviews/' . $model->slug . '?modal=1&mv=0']),
                            'title' => Yii::t('yii', 'See {modelClass}', ['modelClass' => SisReviewsController::CLASS_VERB_NAME]),
                            'class' => 'showModalButton button-as-link',
                ]);
            },
            'update' => function ($url, $model) {
                return Yii::$app->user->identity->administrador < 2 ? '' : Html::button('<span class="fa fa-pen-square"></span> ', [
                            'value' => Url::to(['sis-reviews/' . $model->slug . '?modal=1&mv=' . kartik\detail\DetailView::MODE_EDIT]),
                            'title' => Yii::t('yii', 'Updating {modelClass}', ['modelClass' => SisReviewsController::CLASS_VERB_NAME]),
                            'class' => 'showModalButton button-as-link',
                ]);
                //Html::button('<span class="fa fa-pen-square"></span>', [
                //'value' => Url::to(['sis-reviews/u/' . $model->slug . '?modal=1']),
            ...
            'delete' => function ($url, $model) {
                ...
                $urldelete = Url::home(true) . 'sis-reviews/dlt/' . $model->slug;
...
//                                                }
            ...
            'duplicate' => function ($url, $model) {
                ...
                $url = Url::home(true) . 'sis-reviews/dpl/' . $model->slug;
...
//            'filter' => ArrayHelper::map(SisReviewsController::getDominios(), 'id', 'dominio'),
//            'value' => function($data) {
//                if (in_array($data->dominio, ['mgfolha','MG Folha']) ):
//                    $dominio = Yii::$app->name;
//                else:
//                    $empresa = UserDomains::findOne(['dominio' => $data->dominio]);
//                    $dominio = $empresa['dominio'] . '-' . $empresa['orgao'];
//                endif;
//                return $dominio;
//            },
//        ];
...
//    'contentOptions' => ['style' => 'width: 75px;text-align: right;'],
    ...
    $buttonCreate = Yii::$app->user->identity->administrador < 2 ? '' : Html::button(Yii::t('yii', 'Createa {modelClass}', [
                        'modelClass' => strtolower(Html::encode(SisReviewsController::CLASS_VERB_NAME))
                    ]), [
