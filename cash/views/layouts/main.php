...
// Gera ou garante a existência da pasta de uploads do sistema  
    $path[] = Yii::getAlias('@uploads_root') . "/imagens/$cliente/$dominio/servidores";
    $path[] = Yii::getAlias('@uploads_root') . "/imagens/$cliente/$dominio/usuarios";
...
<html lang="<?= Yii::$app->language ?>">
    ...
    <body onload="$('#loader').hide();" >
        ...
        <div class="wrap">
            <?php
            if (!Yii::$app->user->isGuest) {
                echo $this->render('mainMenu', ['sf' => $sf,]);
                ?>
                <?php
                ...
                ?> 
                <div class="container<?= Yii::$app->mobileDetect->isMobile() ? '' : ('-fluid" ' . (!$sf ? '' : 'style="padding-top: 5%;"')) ?>>
                <?=
                Breadcrumbs::widget([
...
if ((!Yii::$app->mobileDetect->isMobile() ||
    ...
    ?>
    <!-- footer content -->
    <?= $this->render('@pagamentos/views/layouts/footer') ?>
    <!-- /footer content -->
<?php } ?>
<!-- Modal about -->
