...
$menuItemsL[] = ["label" => '<i class="fa fa-edit"></i>' . "Cadastros", "active" => false, "url" => "#",
    "items" => [
        ["label" => "Órgão", "url" => ["/orgao"],
            "visible" => !Yii::$app->user->isGuest && Yii::$app->user->identity->cadastros >= 1,
        ],
        ["label" => "Servidores", "url" => ["/cad-servidores"],
            "visible" => !Yii::$app->user->isGuest && Yii::$app->user->identity->cadastros >= 1,
        ],
    ], "visible" => !Yii::$app->user->isGuest
];
$menuItemsL[] = ["label" => "<i class='fa fa-print'></i>", "active" => false, "url" => "#", 'linkOptions' => ['title' => 'Relatórios e filtros'], "visible" => !Yii::$app->user->isGuest];
...
$menuItemsL[] = (!Yii::$app->user->isGuest && Yii::$app->user->identity->cadastros >= 1) ?
        $this->render('@pagamentos/views/cad-servidores/_search', ['model' => $searchCadModel = new CadServidoresSearch()]) : '';
...
if (!Yii::$app->user->isGuest and 1 == 2) {
    $errosFolha = cash\controllers\FolhaController::getValidaFolha();
    if ($errosFolha > 0) {
...
NavBar::begin([
    'brandLabel' => Html::img(Url::home(true) . Yii::getAlias('@imgs_url') . "/logo_cash-full-no-bg.png", ['width' => '132px']), //"<i class='fa fa-home'></i>",
