...
$menuItemsL = [
    ["label" => '<i class="fa fa-pencil-square-o"></i>' . "Cadastros", "active" => (Yii::$app->controller->id == 'user' || Yii::$app->controller->id == 'empresa' || Yii::$app->controller->id == 'sis-events'), "url" => "#",
        "items" => [
            ...
            ["label" => "SisEvents do sistema", "url" => (!Yii::$app->mobileDetect->isMobile() &&
                !Yii::$app->mobileDetect->isTablet()) ? "#" : ["sis-events"],
                "items" => [
                    ["label" => "Todos", "url" => ["sis-events"]],
                    ["label" => "Hoje", "url" => ["/sis-events?today"]],
                    ["label" => "Esta semana", "url" => ["/sis-events?week"]],
                    ["label" => "Última semana", "url" => ["/sis-events?lastweek"]],
                    ["label" => "Este mês", "url" => ["/sis-events?month"]],
                    ["label" => "Este ano", "url" => ["/sis-events?year"]]
                ],
                "visible" => !Yii::$app->user->isGuest && Yii::$app->user->identity->gestor >= 1
            ],
        ], "visible" => !Yii::$app->user->isGuest
    ],
    ["label" => "<i class='fa fa-calendar'></i> " . "Folha", "active" => (Yii::$app->controller->id == 'user' || Yii::$app->controller->id == 'empresa' || Yii::$app->controller->id == 'sis-events'), "url" => "#", "visible" => !Yii::$app->user->isGuest],
    ["label" => "<i class='fa fa-print'></i> " . "Relatórios", "active" => (Yii::$app->controller->id == 'user' || Yii::$app->controller->id == 'empresa' || Yii::$app->controller->id == 'sis-events'), "url" => "#", "visible" => !Yii::$app->user->isGuest],
    ["label" => "<i class='fa fa-sign-out'></i> " . "Dados externos", "active" => (Yii::$app->controller->id == 'user' || Yii::$app->controller->id == 'empresa' || Yii::$app->controller->id == 'sis-events'), "url" => "#",
        "items" => [
...
$menuItemsR[] = ["label" => "<i class='fa fa-cog'></i> " . "Gestão", "active" => (Yii::$app->controller->id == 'user' || Yii::$app->controller->id == 'empresa' || Yii::$app->controller->id == 'sis-events'), "url" => "#",
    "items" => [
        ["label" => "Usuários", "url" => ["/user"],
            "visible" => !Yii::$app->user->isGuest && Yii::$app->user->identity->gestor >= 1],
        ["label" => "Empresa", "url" => ["/empresa"],
            "visible" => !Yii::$app->user->isGuest && (Yii::$app->user->identity->financeiro >= 3 || Yii::$app->user->identity->gestor >= 1)],
        ["label" => "SisEvents do sistema", "url" => ["sis-events"],
            "visible" => !Yii::$app->user->isGuest && Yii::$app->user->identity->gestor >= 1
        ],
