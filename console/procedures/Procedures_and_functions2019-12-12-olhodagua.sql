...
BEGIN
	SELECT SUM(fr.valor) INTO @receitas 
	...
	SELECT SUM(fr.valor) INTO @descontos 
	FROM fin_rubricas fr 
...
BEGIN
	...
	LEFT JOIN cad_locacoes cl ON cl.id = ff.id_cad_locacoes
	...
	DECLARE cur1Holerites CURSOR FOR  
		SELECT id, liquido FROM temp_holerites;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
	...
	nome_pccs,margemConsignavel,orgao,logradouro,numero,complemento,bairro,cidade,uf,telefone,
	cnpj,mensagem,url_logo) (SELECT ff.id_cad_servidores id_cad_servidor,getLiquidos(cs.dominio,fr.ano,fr.mes,fr.parcela,fr.id_cad_servidores) liquido,"extenso",ff.dominio,ff.ano, ff.mes, ff.parcela,cs.matricula,cs.d_admissao, cs.nome, cs.cpf, cs.rg, cs.pispasep,cb.nome nomeBanco, cs.banco_agencia, cs.banco_agencia_digito, cs.banco_conta, cs.banco_conta_digito, cs.banco_operacao, cl.locacao, cg.nome cargo, cp.id_pccs, cp.nome_pccs, getMargemConsignavel(ff.ano,ff.mes,ff.parcela,cs.id) margemConsignavel, o.orgao, o.logradouro, o.numero, o.complemento, o.bairro, o.cidade, o.uf, o.telefone, o.cnpj, fp.mensagem, o.url_logo
	FROM cad_servidores cs 
	LEFT JOIN cad_bancos cb ON cb.febraban = cs.idbanco
	LEFT JOIN fin_sfuncional ff ON ff.id_cad_servidores = cs.id
	LEFT JOIN fin_rubricas fr ON fr.id_cad_servidores = ff.id_cad_servidores AND fr.mes = ff.mes AND fr.ano = ff.ano AND fr.parcela = ff.parcela
	...
	OPEN cur1Holerites;
	read_loop: LOOP
		FETCH cur1Holerites INTO v_id, v_liquido;
		IF done THEN
			LEAVE read_loop;
		END IF;	
