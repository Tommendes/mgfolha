...
BEGIN
 ...
 SELECT fin_sfuncional.id_pccs,cad_sfuncional.id_vinculo,
	...
	CAST(cad_sfuncional.ano as unsigned) <= CAST(fin_sfuncional.ano parcela as unsigned) and CAST(cad_sfuncional.mes as unsigned) <= CAST(fin_sfuncional.mes as unsigned) AND
...
BEGIN
	...
	JOIN fin_eventos fe ON fe.id = base.id_fin_eventos
	WHERE fe.tipo = 0 AND fe.evento_nome LIKE '%vencimento%' AND base.`id_cad_servidores` = v_id_cad_servidor AND base.ano = CAST(v_ano AS INTEGER) AND 
	base.mes = CAST(v_mes AS INTEGER) AND base.parcela = CAST(v_parcela AS INTEGER);
	
	-- salário bruto
	SELECT SUM(base.valor)base INTO r_bruto FROM fin_rubricas AS base
	JOIN fin_eventos fe ON fe.id = base.id_fin_eventos
	WHERE fe.tipo = 0 AND base.`id_cad_servidores` = v_id_cad_servidor AND base.ano = CAST(v_ano AS INTEGER) AND 
	base.mes = CAST(v_mes AS INTEGER) AND base.parcela = CAST(v_parcela AS INTEGER);
	
	-- descontos
	SELECT SUM(base.valor)base INTO v_descontos FROM fin_rubricas AS base
	JOIN fin_eventos fe ON fe.id = base.id_fin_eventos
	WHERE fe.tipo = 1 AND base.`id_cad_servidores` = v_id_cad_servidor AND base.ano = CAST(v_ano AS INTEGER) AND 
	base.mes = CAST(v_mes AS INTEGER) AND base.parcela = CAST(v_parcela AS INTEGER);
...
BEGIN
	...
	SELECT fin_sfuncional.id_pccs,cad_sfuncional.id_vinculo,
		...
		JOIN cad_servidores ON cad_sfuncional.id_cad_servidores = cad_servidores.id
		JOIN fin_sfuncional ON cad_sfuncional.id_cad_servidores = fin_sfuncional.id_cad_servidores AND 
		CAST(cad_sfuncional.parcela as unsigned) <= CAST(fin_sfuncional.ano parcela as unsigned) and CAST(cad_sfuncional.mes as unsigned) <= CAST(fin_sfuncional.mes as unsigned) AND
		CAST(cad_sfuncional.parcela as unsigned) <= CAST(fin_sfuncional.parcela as unsigned)
...
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;
>>>>>>> 6ab62a8cc4a64dc5ed3ae0bcb2d08d2e168a71b4
