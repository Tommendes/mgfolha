...
MySQL - 10.4.24-MariaDB : Database - mgfolha_folha
*********************************************************************
*/
...
/* Function  structure for function  `getValorPorExtenso` */

/*!50003 DROP FUNCTION IF EXISTS `getValorPorExtenso` */;
DELIMITER $$

/*!50003 CREATE FUNCTION `getValorPorExtenso`(valor double(11,2)) RETURNS varchar(255) CHARSET utf8mb4
...
DECLARE grupos INT DEFAULT 0;
	...
	read_loop: LOOP
		...
		END IF;	
	END LOOP;
	IF valor IS NULL OR CAST(valor AS DOUBLE) = 0 THEN
		SET @rt = "zero";
	END IF;
...
/* Procedure structure for procedure `getBancosDeDados` */

/*!50003 DROP PROCEDURE IF EXISTS  `getBancosDeDados` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `getBancosDeDados`()
    COMMENT 'Retorna os nomes dos bancos de dados baseado na coluna cliente da tabela user'
BEGIN
	SELECT CAST(CONCAT('mgfolha_',LOWER(REPLACE(u.cliente,'_', ''))) AS CHAR) bancosDeDados, u.cliente clientes FROM `user` u WHERE u.`cliente` IS NOT NULL AND LENGTH(u.`cliente`) > 1 GROUP BY u.`cliente`;
    END */$$
DELIMITER ;
...
/* Procedure structure for procedure `getServidorByToken` */

/*!50003 DROP PROCEDURE IF EXISTS  `getServidorByToken` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `getServidorByToken`(token VARCHAR(255), resto varchar (255))
    COMMENT 'Retorna o token, CPF e matricula baseado num token informado'
BEGIN
	SELECT token, SUBSTR(CONV(token,36,12),length(resto)+1,11) cpf,SUBSTR(CONV(token,36,12),13) matricula, SUBSTR(CONV(token,36,12),LENGTH(resto),LENGTH(resto)) resto;
    END */$$
DELIMITER ;

/* Procedure structure for procedure `getTokenValidacao` */

/*!50003 DROP PROCEDURE IF EXISTS  `getTokenValidacao` */;

DELIMITER $$

/*!50003 CREATE PROCEDURE `getTokenValidacao`(token varchar(255))
    COMMENT 'Retorna os parâmetros do token de validação informado'
BEGIN
	SELECT SUBSTRING(FROM_BASE64(token),1,4)ano,
		SUBSTRING(FROM_BASE64(token),5,2)mes,
		SUBSTRING(FROM_BASE64(token),7,3)parcela,
		CAST(SUBSTRING(FROM_BASE64(token),10,10) AS CHAR(10))id_cad_servidor,
		CONCAT("mgfolha_",SUBSTRING(FROM_BASE64(token),20))cliente;
    END */$$
DELIMITER ;
